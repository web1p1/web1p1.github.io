function newElement() {
    ...
    sessionStorage.setItem(number.toString(), myListContent.value + '0');  // 초기입력 0
...
function checkeLine(number) {  // 정수입력 
    ...
    let val = sessionStorage.getItem(number);
    let value = val.substr(0, val.length - 1);
    let checkedNumber = parseInt(val.substr(-1,));  // 맨마지막 문자 (0또는 1)
    // 홀수로 만들어 줌 (완료된 리스트)
    if (checkedNumber == 0) { // 체크안된것
        sessionStorage.setItem(number.toString(), value + "1");
    }
    else { // 체크된 것
        sessionStorage.setItem(number.toString(), value + "0");
    }
}
...
function deleteLine(number) {
    ...
    sessionStorage.removeItem(number.toString());  // 삭제
}
